...
export const generatingLoaderStyle = css `
    ...
    :host {
    --_skeleton-gradient1: #{$magi-primary-light};
    --_skeleton-gradient2: #{$magi-primary};
    background: none;

    }

    :host([spk2]) {
    --_skeleton-gradient1: #{$color-surface-container-high};
    --_skeleton-gradient2: #{$color-primary};
    ...
    .text-container {
    color: $vais-grey-600;
    @include mixins.ellipsify($widget-width);
    }
    ...
    width: min($widget-width, 100%);
    }

    .second-line.skeleton-loader {
    width: min(
        $widget-width * $second-bar-ratio,
        math.percentage($second-bar-ratio)
    );
    }
